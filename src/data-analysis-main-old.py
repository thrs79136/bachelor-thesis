...
from src.helper.genres import create_genres_dict, create_genres_df, transform_genre_to_string, transform_genre_string
...
from src.helper.knn.k_nearest_neighbor import k_nearest_neighbor, k_nearest_neighbor_genre, \
    k_nearest_neighbor_decade_all_features, k_nearest_neighbor_all_decades_all_features, \
    k_nearest_neighbor_before_90s_all_features, knn_genre_mcgill, knn_genre_spotify
from src.helper.knn.knn_n_minus_one_training_records import knn_n_minus_one_mcgill_decade, knn_n_minus_one_mcgill_genre, \
    knn_n_minus_one_genre_spotify, knn_n_minus_one_decade_spotify
from src.helper.knn.knn_regression import knn_regression, knn_regression_chart_pos_ds1, \
    knn_regression_spotify_popularity_ds1, knn_regression_year_ds1, knn_regression_year_ds1_balanced
from src.helper.file_helper import get_songs_from_binary_file, get_songs, save_feature_csv, get_songs_from_feature_csv, \
    save_median_feature_csv, save_dataframe, feature_file_path
...
from src.helper.statistics.best_features_analyzer import get_best_features, get_best_features_years
...
from src.helper.svm.svm import svm_decade_mcgill, svm_genre_mcgill, svm_genre_spotify, svm_decade_spotify
...
def start_parallel_coordinate_creation(filepath, filename):
    create_parallel_coordinates_plot_newest(feature_list, filepath, filename)
...
def get_most_popular_song_per_year(songs: List[Song], count=1):
    most_popular_songs = []

    years_group = group_by_year(songs)
    for song_list in years_group.values():
        print(len(song_list))
        sorted_by_popularity = sorted(song_list, key=lambda item: item.get_spotify_popularity(), reverse=True)
        # add up to count songs for every year
        for i in range(min(count, len(song_list))):
            most_popular_songs.append(sorted_by_popularity[i])
...
#create_barplot([0.9, 0.8, 0.7, 0.5], ['A', 'B', 'C', 'D'], 'dummy1.png', 'title', 'subtitle')
#create_barplot([0.9, 0.6, 0.7, 1], ['A', 'B', 'C', 'D'], 'dummy2.png', 'title', 'subtitle')

#knn_n_minus_one_mcgill_genre()
# knn_n_minus_one_mcgill_decade()
# knn_n_minus_one_genre_spotify()
# knn_n_minus_one_decade_spotify()
# knn_regression_spotify_popularity_ds1()
# knn_regression_chart_pos_ds1()
# knn_regression_year_ds1()

def get_chord_with_most_notes(songs):
    chord_notes_max = 0
    for song in songs:
        for section in song.mcgill_billboard_song_data.sections:
            for chord in section.get_chords():
                chord_notes_max = max(len(chord.notes), chord_notes_max)
    return chord_notes_max

# knn_regression_year_ds1_balanced()
# exit()
...
# k_nearest_neighbor_all_decades_all_features()
# k_nearest_neighbor_before_90s_all_features()
# exit()
...
ret = get_chord_with_most_notes(songs_with_audio_features)
...
most_popular_songs = get_most_popular_song_per_year(songs_with_audio_features, 10)
analyze_instruments(most_popular_songs, 'instruments_most_popular')
exit()

draw_genres_area_plot_new(most_popular_songs, 'genre_perc_most_popular.png')
exit()


most_popular_songs = get_most_popular_song_per_year(songs_with_audio_features, 2)

save_feature_csv(most_popular_songs, list(song_features.song_features_dict.keys()), '../data/csv/features_most_popular2.csv')
start_parallel_coordinate_creation('../data/csv/features_most_popular2.csv', 'most_popular2.png')
